...
namespace RE {
	...
	bool init_render_element_rectangle(Vulkan_Buffer &rStagingRectBuffer, VulkanTask *pTransferTask, Vulkan_Fence &rTransferFence) {
		...
		if (rStagingRectBuffer.create(0, RE_VK_RECT_BUFFER_TOTAL_SIZE, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, 1, nullptr, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)) {
			...
			if (rStagingRectBuffer.get_memory().map(0, RE_VK_RECT_BUFFER_VERTICES_OFFSET, RE_VK_RECT_BUFFER_VERTICES_SIZE, reinterpret_cast<void**>(&pafVertices))) {
				...
				if (rStagingRectBuffer.get_memory().map(0, RE_VK_RECT_BUFFER_INDICES_OFFSET, RE_VK_RECT_BUFFER_INDICES_SIZE, reinterpret_cast<void**>(&pau16Indices))) {
					...
					if (create_vulkan_buffer(0, RE_VK_RECT_BUFFER_TOTAL_SIZE, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, 1, nullptr, &vk_hRectBuffer)) {
						if (rectBufferMemory.alloc_for_buffer(vk_hRectBuffer, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) == VK_SUCCESS) {
