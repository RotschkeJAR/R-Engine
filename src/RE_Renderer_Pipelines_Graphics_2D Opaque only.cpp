...
namespace RE {
	...
	VkPipeline vk_hGraphicsPipeline2DOpaqueOnly = VK_NULL_HANDLE;

	bool create_graphics_pipeline_2D_opaque_only(const VkShaderModule vk_hVertexShader, const VkShaderModule vk_hFragmentShader) {
		PRINT_DEBUG("Creating Vulkan graphics pipeline for 2D opaque-only rendering");
		const VkPipeline vk_hPreviousPipeline = vk_hGraphicsPipeline2DOpaqueOnly;
		...
		if (vkCreateGraphicsPipelines(vk_hDevice, VK_NULL_HANDLE, 1, &vk_createInfo, nullptr, &vk_hGraphicsPipeline2DOpaqueOnly) == VK_SUCCESS) {
			PRINT_DEBUG("Destroying old Vulkan graphics pipeline ", vk_hPreviousPipeline, " used for 2D opaque-only rendering");
			vkDestroyPipeline(vk_hDevice, vk_hPreviousPipeline, nullptr);
			return true;
		} else
			RE_ERROR("Failed to create Vulkan graphics pipeline used for 2D opaque-only rendering");
		return false;
	}

	void destroy_graphics_pipeline_2D_opaque_only() {
		PRINT_DEBUG("Destroying Vulkan graphics pipeline ", vk_hGraphicsPipeline2DOpaqueOnly, " used for 2D opaque-only rendering");
		vkDestroyPipeline(vk_hDevice, vk_hGraphicsPipeline2DOpaqueOnly, nullptr);
		vk_hGraphicsPipeline2DOpaqueOnly = VK_NULL_HANDLE;
	}
