...
# define RE_WM_MAXIMIZED WM_APP
		...
		if (win_hWindow != win_hWndParam && bRunning)
			...
			switch (win_uMsg) {
				...
				case WM_GETMINMAXINFO: {
					...
					win_monitorInfoRetrieved = GetMonitorInfoW(MonitorFromWindow(win_hWndParam, MONITOR_DEFAULTTOPRIMARY), &win_monitorInfo);
	...
	void win64_show_window() {
		ShowWindow(win_hWindow, are_bits_true<uint8_t>(u8WindowFlagBits, WINDOW_VISIBLE_BIT));
	}
