...
namespace RE {
	...
	std::unique_ptr<uint32_t[]> pau32QueueIndices;
...
#define QUEUE_INDEX_GENERAL_COMPUTING 3
	...
	void create_queue_create_infos(const float *pfPriority, std::vector<VkDeviceQueueCreateInfo> &vk_rpaLogicalQueueCreateInfos) {
		...
		vkGetPhysicalDeviceQueueFamilyProperties(get_selected_physical_vulkan_device(), &u32QueueFamilyCount, nullptr);
		std::vector<VkQueueFamilyProperties> vk_paQueueFamilyProperties;
		vk_paQueueFamilyProperties.resize(u32QueueFamilyCount);
		vkGetPhysicalDeviceQueueFamilyProperties(get_selected_physical_vulkan_device(), &u32QueueFamilyCount, vk_paQueueFamilyProperties.data());
		...
		for (uint32_t i = 0; i < u32QueueFamilyCount; i++) {
			...
			VkBool32 vk_bPresentingSupported;
			vkGetPhysicalDeviceSurfaceSupportKHR(get_selected_physical_vulkan_device(), i, vk_hSurface, &vk_bPresentingSupported);
		...
		for (const uint32_t u32SelectedQueueIndex : selectedQueues) {
			VkBool32 vk_bPresentingSupported;
			vkGetPhysicalDeviceSurfaceSupportKHR(get_selected_physical_vulkan_device(), u32SelectedQueueIndex, vk_hSurface, &vk_bPresentingSupported);
			if (vk_bPresentingSupported == VK_TRUE) {
		...
		pau32QueueIndices = std::make_unique<uint32_t[]>(u8LogicalQueueCount);
		...
		for (const uint32_t u32QueueIndex : selectedQueues) {
			...
			VkBool32 vk_bPresentingSupported;
			vkGetPhysicalDeviceSurfaceSupportKHR(get_selected_physical_vulkan_device(), u32LogicalQueueCreateIndex, vk_hSurface, &vk_bPresentingSupported);
			if (vk_bPresentingSupported == VK_TRUE)
