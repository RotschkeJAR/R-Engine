...
namespace RE {
	...
	void Vulkan_Queue::submit_to_present_queue(const VkPresentInfoKHR *const vk_pPresentInfo) const {
		CATCH_SIGNAL(vkQueuePresentKHR(vk_hQueue, vk_pPresentInfo));
	}

	void Vulkan_Queue::submit_to_present_queue(const uint32_t u32SemaphoresToWaitForCount, const VkSemaphore *vk_phSemaphoresToWaitFor, const uint32_t u32SwapchainCount, const VkSwapchainKHR *vk_phSwapchains, const uint32_t *pu32SwapchainImageIndices) const {
	...
	void Vulkan_Queue::submit_to_present_queue(const uint32_t u32SemaphoresToWaitForCount, const Vulkan_Semaphore *pSemaphoresToWaitFor, const uint32_t u32SwapchainCount, const VkSwapchainKHR *vk_phSwapchains, const uint32_t *pu32SwapchainImageIndices) const {
		VkSemaphore *const vk_phSemaphoresToWaitFor = new VkSemaphore[u32SemaphoresToWaitForCount];
	...
	void Vulkan_Queue::wait_for_idle_queue() const {
		CATCH_SIGNAL(vkQueueWaitIdle(vk_hQueue));
	}

	VkQueue Vulkan_Queue::get_queue() const {
		return vk_hQueue;
	}

	bool Vulkan_Queue::is_valid() const {
		return vk_hQueue != VK_NULL_HANDLE;
	}

	Vulkan_Queue::operator VkQueue() const {
