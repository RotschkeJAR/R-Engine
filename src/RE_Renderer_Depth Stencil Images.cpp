...
namespace RE {
	...
	VkImage vk_a2hDepthStencilImages[2];
	VkDeviceMemory vk_a2hDepthStencilImageMemories[2];
	VkImageView vk_a4hDepthStencilImageViews[4];
	std::vector<VkFormat> availableDepthStencilFormats;
	...
	bool does_gpu_support_depth_stencil_images(const VkPhysicalDevice vk_hPhysicalDevice, std::queue<std::string> &rMissingFeatures) {
		...
		for (const VkFormat vk_eDesiredFormat : vk_aeDesiredFormats) {
			PRINT_DEBUG("Checking physical Vulkan device's supported features for format ", std::hex, vk_eDesiredFormat);
			...
			if ((vk_formatProperties.formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) != VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT)
				continue;
			PRINT_DEBUG("Checking physical Vulkan device's format properties");
			...
			if (vk_imageFormatProperties.imageFormatProperties.maxExtent.width < largestMonitorSize[0] || vk_imageFormatProperties.imageFormatProperties.maxExtent.height < largestMonitorSize[1])
				continue;
			PRINT_DEBUG("Physical Vulkan device passed requirements to support the format");
		...
		PRINT_DEBUG("Physical Vulkan device doesn't support all essential requirements for depth/stencil images");
	...
	int32_t rate_gpu_depth_stencil_image_formats(const VkPhysicalDevice vk_hPhysicalDevice, VkSampleCountFlags &vk_reMsaaAvailable) {
		PRINT_DEBUG("Rating physical Vulkan device's (", vk_hPhysicalDevice, ") capabilities for depth/stencil images");
		int32_t i32Score = 0;
		...
		for (const VkFormat vk_eFormat : vk_aeAllFormats) {
			PRINT_DEBUG("Checking physical Vulkan device's supported features for format ", std::hex, vk_eFormat);
			vkGetPhysicalDeviceFormatProperties2(vk_hPhysicalDevice, vk_eFormat, &vk_formatProperties);
			if ((vk_formatProperties.formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) != VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT)
				continue;
			PRINT_DEBUG("Querying physical Vulkan device's format properties");
			vk_physicalDeviceDepthStencilImageFormatInfo.format = vk_eFormat;
			vkGetPhysicalDeviceImageFormatProperties2(vk_hPhysicalDevice, &vk_physicalDeviceDepthStencilImageFormatInfo, &vk_imageFormatProperties);
			if (vk_imageFormatProperties.imageFormatProperties.maxExtent.width < largestMonitorSize[0] || vk_imageFormatProperties.imageFormatProperties.maxExtent.height < largestMonitorSize[1])
				continue;
			PRINT_DEBUG("Filtering unavailable MSAA-options for format out");
		...
		if ((!bSupportsDepthStencilCombined && bSupportsStencilsOnly && bSupportsDepthOnly) || (bSupportsDepthStencilCombined && !(bSupportsStencilsOnly && bSupportsDepthOnly))) {
			PRINT_DEBUG("Lowering score because of bad GPU design spotted");
			i32Score /= 5;
		}
		PRINT_DEBUG("Final score for rating depth/stencil image-support: ", i32Score);
		return i32Score;
	}
	...
	void find_suitable_depth_stencil_formats() {
		...
		PRINT_DEBUG("Querying properties about stencil-only Vulkan format");
		vkGetPhysicalDeviceFormatProperties2(vk_hPhysicalDeviceSelected, VK_FORMAT_S8_UINT, &vk_formatProperties);
		if ((vk_formatProperties.formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) == VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) {
			PRINT_DEBUG("Querying properties about image format using stencil-only");
			...
			bSeparateStencilsSupported = vk_imageFormatProperties.imageFormatProperties.maxExtent.width >= largestMonitorSize[0] && vk_imageFormatProperties.imageFormatProperties.maxExtent.height >= largestMonitorSize[1];
		} else
			bSeparateStencilsSupported = false;
		PRINT_DEBUG("A separate Vulkan stencil image is supported: ", bSeparateStencilsSupported);
		...
		for (const VkFormat vk_eDepthStencilFormat : vk_aeDepthStencilCombinedFormats) {
			PRINT_DEBUG("Querying properties about Vulkan format ", std::hex, vk_eDepthStencilFormat);
			vkGetPhysicalDeviceFormatProperties2(vk_hPhysicalDeviceSelected, vk_eDepthStencilFormat, &vk_formatProperties);
			if ((vk_formatProperties.formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) == VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) {
				PRINT_DEBUG("Querying image properties using the Vulkan format");
				vk_physicalDeviceImageFormatInfo.format = vk_eDepthStencilFormat;
				vkGetPhysicalDeviceImageFormatProperties2(vk_hPhysicalDeviceSelected, &vk_physicalDeviceImageFormatInfo, &vk_imageFormatProperties);
				if (vk_imageFormatProperties.imageFormatProperties.maxExtent.width >= largestMonitorSize[0] && vk_imageFormatProperties.imageFormatProperties.maxExtent.height >= largestMonitorSize[1]) {
					PRINT_DEBUG("Depth format fulfills requirements and can be used");
					availableDepthStencilFormats.push_back(vk_eDepthStencilFormat);
				}
			}
		}
		PRINT_DEBUG("Shrinking list of suitable Vulkan formats for depth/stencil images");
	...
	void discard_incompatible_msaa_modes_for_depth_stencil_images(VkSampleCountFlags &vk_reMsaaAvailable) {
		...
		for (const VkFormat vk_eFormat : availableDepthStencilFormats) {
			PRINT_DEBUG("Querying supported sample count for Vulkan depth/stencil image format ", std::hex, vk_eFormat);
	...
	bool create_depth_stencil_images(VulkanTask &rDepthStencilImageLayoutTransitionTask, VkFence vk_hDepthStencilImageLayoutTransitionFence) {
		if (!bStencilsEnabled && std::none_of(availableDepthStencilFormats.rbegin(), availableDepthStencilFormats.rend(), 
			...
			RE_WARNING("The selected GPU doesn't support any format to use depth images only. The engine will use a depth-stencil-combined image format instead, which might result in reserving unused memory");
			bStencilsEnabled = true;
		}
		VkExtent3D vk_depthStencilImageExtent3D {
		...
		rDepthStencilImageLayoutTransitionTask.init(1, a1u8RenderGraphicsLogicalQueueIndex, false, true);
		if (bStencilsEnabled) {
			...
			if (are_depth_stencil_images_separated(vk_eSelectedDepthFormat)) {
				if (bSeparateStencilsSupported) {
					PRINT_DEBUG("Creating Vulkan depth image array in format ", std::hex, vk_eSelectedDepthFormat);
					if (create_vulkan_image(0, VK_IMAGE_TYPE_2D, vk_eSelectedDepthFormat, vk_depthStencilImageExtent3D, 1, RE_VK_FRAMES_IN_FLIGHT, vk_eMsaaCount, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 1, nullptr, VK_IMAGE_LAYOUT_UNDEFINED, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &vk_a2hDepthStencilImages[0], &vk_a2hDepthStencilImageMemories[0])) {
						if (create_vulkan_image(0, VK_IMAGE_TYPE_2D, VK_FORMAT_S8_UINT, vk_depthStencilImageExtent3D, 1, RE_VK_FRAMES_IN_FLIGHT, vk_eMsaaCount, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 1, nullptr, VK_IMAGE_LAYOUT_UNDEFINED, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &vk_a2hDepthStencilImages[1], &vk_a2hDepthStencilImageMemories[1])) {
							uint8_t u8RenderInFlightImageCreateIndex = 0;
							while (u8RenderInFlightImageCreateIndex < RE_VK_FRAMES_IN_FLIGHT) {
								PRINT_DEBUG("Creating Vulkan image view for depth image at index ", u8RenderInFlightImageCreateIndex);
								if (create_vulkan_image_view(vk_a2hDepthStencilImages[0], VK_IMAGE_VIEW_TYPE_2D, vk_eSelectedDepthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 0, 1, u8RenderInFlightImageCreateIndex, 1, &vk_a4hDepthStencilImageViews[u8RenderInFlightImageCreateIndex])) {
									PRINT_DEBUG("Creating Vulkan image view for stencil image at index ", u8RenderInFlightImageCreateIndex);
									if (create_vulkan_image_view(vk_a2hDepthStencilImages[1], VK_IMAGE_VIEW_TYPE_2D, VK_FORMAT_S8_UINT, VK_IMAGE_ASPECT_STENCIL_BIT, 0, 1, u8RenderInFlightImageCreateIndex, 1, &vk_a4hDepthStencilImageViews[2 + u8RenderInFlightImageCreateIndex])) {
									...
									PRINT_DEBUG("Destroying Vulkan depth image view due to failure creating a stencil image view before");
									vkDestroyImageView(vk_hDevice, vk_a4hDepthStencilImageViews[u8RenderInFlightImageCreateIndex * 2], nullptr);
								} else
							...
							if (u8RenderInFlightImageCreateIndex < RE_VK_FRAMES_IN_FLIGHT) {
								for (uint8_t u8RenderInFlightImageDestroyIndex = 0; u8RenderInFlightImageDestroyIndex < u8RenderInFlightImageCreateIndex; u8RenderInFlightImageDestroyIndex++) {
									PRINT_DEBUG("Destroying Vulkan image views at frame-in-flight index ", u8RenderInFlightImageDestroyIndex, " due to failure creating essential depth/stencil image/view");
									vkDestroyImageView(vk_hDevice, vk_a4hDepthStencilImageViews[2 + u8RenderInFlightImageDestroyIndex], nullptr);
									vkDestroyImageView(vk_hDevice, vk_a4hDepthStencilImageViews[u8RenderInFlightImageDestroyIndex], nullptr);
								}
								return false;
							} else {
								PRINT_DEBUG("Recording Vulkan depth/stencil image layout transitioning-task");
								rDepthStencilImageLayoutTransitionTask.record(0, VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT, [&](const VkCommandBuffer vk_hCommandBuffer, const uint8_t u8PreviousLogicalQueue, const uint8_t u8CurrentLogicalQueue, const uint8_t u8NextLogicalQueue) {
									PRINT_DEBUG("Transferring image layout of depth/stencil images to depth/stencil optimal layout");
									const VkImageMemoryBarrier2 vk_aDepthStencilImageBarriers[2] = {
										{
											...
											.image = vk_a2hDepthStencilImages[0],
											.subresourceRange = {
												.aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT,
												.levelCount = 1,
												.layerCount = RE_VK_FRAMES_IN_FLIGHT
											}
										}, {
											.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2,
											.srcStageMask = VK_PIPELINE_STAGE_2_NONE,
											...
											.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
											.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
											.image = vk_a2hDepthStencilImages[1],
											.subresourceRange = {
								...
								PRINT_DEBUG("Submitting depth/stencil image layout transitioning-task to GPU");
								rDepthStencilImageLayoutTransitionTask.submit(0, nullptr, nullptr, 0, nullptr, vk_hDepthStencilImageLayoutTransitionFence);
								return true;
							}
							PRINT_DEBUG("Destroying Vulkan stencil image due to failure creating an essential depth/stencil image/view");
							vkFreeMemory(vk_hDevice, vk_a2hDepthStencilImageMemories[1], nullptr);
							vkDestroyImage(vk_hDevice, vk_a2hDepthStencilImages[1], nullptr);
						} else
							RE_FATAL_ERROR("Failed to create separated Vulkan stencil image");
						PRINT_DEBUG("Destroying Vulkan depth image due to failure creating an essential depth/stencil image/view");
						...
						RE_FATAL_ERROR("Failed to create separated Vulkan depth image");
					return false;
				} else {
					...
					for (uint8_t u8FormatIndex = 0; u8FormatIndex < availableDepthStencilFormats.size(); u8FormatIndex++) {
						PRINT_DEBUG("Trying to find unseparated Vulkan depth/stencil format at index ", u8FormatIndex);
						if (!are_depth_stencil_images_separated(availableDepthStencilFormats[u8FormatIndex])) {
							...
							PRINT_DEBUG("New unseparated Vulkan depth/stencil format ", std::hex, vk_eSelectedDepthFormat, " at index ", std::dec, u8IndexToSelectedDepthStencilFormat, " selected");
							break;
						}
					}
			...
			PRINT_DEBUG("Creating Vulkan depth-stencil-combined image");
			if (create_vulkan_image(0, VK_IMAGE_TYPE_2D, vk_eSelectedDepthFormat, vk_depthStencilImageExtent3D, 1, RE_VK_FRAMES_IN_FLIGHT, vk_eMsaaCount, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 1, nullptr, VK_IMAGE_LAYOUT_UNDEFINED, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &vk_a2hDepthStencilImages[0], &vk_a2hDepthStencilImageMemories[0])) {
				uint8_t u8DepthStencilImageViewCreateIndex = 0;
				while (u8DepthStencilImageViewCreateIndex < RE_VK_FRAMES_IN_FLIGHT) {
					PRINT_DEBUG("Creating Vulkan image view for depth-stencil-combined image at index ", u8DepthStencilImageViewCreateIndex);
					if (create_vulkan_image_view(vk_a2hDepthStencilImages[0], VK_IMAGE_VIEW_TYPE_2D, vk_eSelectedDepthFormat, VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT, 0, 1, u8DepthStencilImageViewCreateIndex, 1, &vk_a4hDepthStencilImageViews[u8DepthStencilImageViewCreateIndex])) {
						 u8DepthStencilImageViewCreateIndex++;
						continue;
					} else
						RE_FATAL_ERROR("Failed to create Vulkan image view for depth-stencil-combined image at index ", u8DepthStencilImageViewCreateIndex);
				}
				if (u8DepthStencilImageViewCreateIndex != RE_VK_FRAMES_IN_FLIGHT) {
					...
					PRINT_DEBUG("Recording Vulkan depth-stencil-combined image layout transitioning-task");
					rDepthStencilImageLayoutTransitionTask.record(0, VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT, [&](const VkCommandBuffer vk_hCommandBuffer, const uint8_t u8PreviousLogicalQueue, const uint8_t u8CurrentLogicalQueue, const uint8_t u8NextLogicalQueue) {
						PRINT_DEBUG("Transferring image layout of Vulkan depth-stencil-combined images on GPU");
						const VkImageMemoryBarrier2 vk_depthStencilImageBarriers = {
							.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2,
							.srcStageMask = VK_PIPELINE_STAGE_2_NONE,
							.srcAccessMask = VK_ACCESS_2_NONE,
							.dstStageMask = VK_PIPELINE_STAGE_2_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_2_LATE_FRAGMENT_TESTS_BIT,
						...
						const VkDependencyInfo vk_imageTransitionBarrier = {
							...
							.pImageMemoryBarriers = &vk_depthStencilImageBarriers
						};
						vkCmdPipelineBarrier2(vk_hCommandBuffer, &vk_imageTransitionBarrier);
					});
					PRINT_DEBUG("Submitting Vulkan depth image layout transitioning-task");
					rDepthStencilImageLayoutTransitionTask.submit(0, nullptr, nullptr, 0, nullptr, vk_hDepthStencilImageLayoutTransitionFence);
					return true;
				}
			} else
				RE_FATAL_ERROR("Failed to create Vulkan depth-stencil-combined image");
		} else [[likely]] {
			...
			PRINT_DEBUG("Creating Vulkan depth-only image");
			if (create_vulkan_image(0, VK_IMAGE_TYPE_2D, availableDepthStencilFormats[u8IndexToSelectedDepthStencilFormat], vk_depthStencilImageExtent3D, 1, RE_VK_FRAMES_IN_FLIGHT, vk_eMsaaCount, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 1, nullptr, VK_IMAGE_LAYOUT_UNDEFINED, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &vk_a2hDepthStencilImages[0], &vk_a2hDepthStencilImageMemories[0])) {
				uint8_t u8DepthImageViewCreateIndex = 0;
				while (u8DepthImageViewCreateIndex < RE_VK_FRAMES_IN_FLIGHT) {
					PRINT_DEBUG("Creating Vulkan image view for depth-only image at index ", u8DepthImageViewCreateIndex);
					if (create_vulkan_image_view(vk_a2hDepthStencilImages[0], VK_IMAGE_VIEW_TYPE_2D, availableDepthStencilFormats[u8IndexToSelectedDepthStencilFormat], VK_IMAGE_ASPECT_DEPTH_BIT, 0, 1, u8DepthImageViewCreateIndex, 1, &vk_a4hDepthStencilImageViews[u8DepthImageViewCreateIndex])) {
						u8DepthImageViewCreateIndex++;
						continue;
					} else
						RE_FATAL_ERROR("Failed to create Vulkan image view for depth image at index ", u8DepthImageViewCreateIndex);
					break;
				}
				if (u8DepthImageViewCreateIndex < RE_VK_FRAMES_IN_FLIGHT) {
					...
					PRINT_DEBUG("Recording Vulkan depth image layout transitioning-task");
					rDepthStencilImageLayoutTransitionTask.record(0, VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT, [&](const VkCommandBuffer vk_hCommandBuffer, const uint8_t u8PreviousLogicalQueue, const uint8_t u8CurrentLogicalQueue, const uint8_t u8NextLogicalQueue) {
						PRINT_DEBUG("Transferring image layout of Vulkan depth image");
						const VkImageMemoryBarrier2 vk_depthImageBarriers = {
							.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2,
							.srcStageMask = VK_PIPELINE_STAGE_2_NONE,
							.srcAccessMask = VK_ACCESS_2_NONE,
							.dstStageMask = VK_PIPELINE_STAGE_2_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_2_LATE_FRAGMENT_TESTS_BIT,
						...
						const VkDependencyInfo vk_imageTransitionBarrier = {
							.sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO,
							.imageMemoryBarrierCount = 1,
							.pImageMemoryBarriers = &vk_depthImageBarriers
						};
						vkCmdPipelineBarrier2(vk_hCommandBuffer, &vk_imageTransitionBarrier);
					});
					PRINT_DEBUG("Submitting Vulkan depth image layout transitioning-task");
				...
				RE_FATAL_ERROR("Failed to create Vulkan depth image");
		}
		return false;
	}

	void destroy_depth_stencil_images() {
		const uint8_t u8ImageCount = (bStencilsEnabled && are_depth_stencil_images_separated(availableDepthStencilFormats[u8IndexToSelectedDepthStencilFormat])) ? 1 : 2;
		PRINT_DEBUG("Number of Vulkan depth/stencil images, that have to be destroyed: ", u8ImageCount);
		for (uint8_t u8ImageIndex = 0; u8ImageIndex < u8ImageCount; u8ImageIndex++) {
			PRINT_DEBUG("Destroying Vulkan depth/stencil image and its image view at index ", u8ImageIndex);
			vkDestroyImageView(vk_hDevice, vk_a4hDepthStencilImageViews[u8ImageIndex * 2], nullptr);
