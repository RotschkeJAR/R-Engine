...
namespace RE {
	...
	bool alloc_memory_for_images_renderer() {
		...
		if (alloc_shared_vulkan_memory(static_cast<uint32_t>(imageInfos.size()), imageInfos.data(), VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, memoryAllocationCount, imageMemories))
			return true;
		else
