...
#define vk_wait_for_and_reset_fence(FENCE) CATCH_SIGNAL(__vk_wait_for_and_reset_fence(FENCE))
	...
	void __vk_signal_semaphores(const uint32_t u32SignalSemaphoreCount, const VkSemaphore *vk_phSignalSemaphores, const char *pcFile, const char *pcFunc, const uint32_t u32Line);
#define vk_signal_semaphores(SIGNAL_SEMA_COUNT, SIGNAL_SEMAS) __vk_signal_semaphores(SIGNAL_SEMA_COUNT, SIGNAL_SEMAS, __FILE__, __func__, __LINE__)
...
#define vk_submit_to_graphics_queue(WAIT_SEMA_COUNT, WAIT_SEMAS, WAIT_PIPES, CMDBUFFER_COUNT, CMDBUFFERS, SIGNAL_SEMA_COUNT, SIGNAL_SEMAS, SIGNAL_FENCE) __vk_submit_to_graphics_queue(WAIT_SEMA_COUNT, WAIT_SEMAS, WAIT_PIPES, CMDBUFFER_COUNT, CMDBUFFERS, SIGNAL_SEMA_COUNT, SIGNAL_SEMAS, SIGNAL_FENCE, __FILE__, __func__, __LINE__)

	bool __vk_submit_to_transfer_queue(const uint32_t u32WaitSemaphoreCount, const VkSemaphore *vk_phWaitSemaphores, const VkPipelineStageFlagBits vk_eWaitPipelineStage, const uint32_t u32CommandBufferCount, const VkCommandBuffer *vk_phCommandBuffers, const uint32_t u32SignalSemaphoreCount, const VkSemaphore vk_phSignalSemaphores, const VkFence vk_hFenceToSignal, const char *pcFile, const char *pcFunc, const uint32_t u32Line);
#define vk_submit_to_transfer_queue(WAIT_SEMA_COUNT, WAIT_SEMAS, WAIT_PIPES, CMDBUFFER_COUNT, CMDBUFFERS, SIGNAL_SEMA_COUNT, SIGNAL_SEMAS, SIGNAL_FENCE) __vk_submit_to_transfer_queue(WAIT_SEMA_COUNT, WAIT_SEMAS, WAIT_PIPES, CMDBUFFER_COUNT, CMDBUFFERS, SIGNAL_SEMA_COUNT, SIGNAL_SEMAS, SIGNAL_FENCE, __FILE__, __func__, __LINE__)
...
#define vk_submit_to_present_queue(WAIT_SEMA_COUNT, WAIT_SEMAS, SWAPIMG_INDEX) __vk_submit_to_present_queue(WAIT_SEMA_COUNT, WAIT_SEMAS, SWAPIMG_INDEX, __FILE__, __func__, __LINE__)
