...
namespace RE {
	...
	Texture alloc_texture_from_binary_data(const uint8_t *const pau8TextureBinaries, const uint32_t u32Width, const uint32_t u32Height, const uint32_t u32Channels) {
		...
		Vulkan_Buffer stagingImageBuffer(0, vk_imageBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, 1, nullptr, RE_VK_CPU_RAM, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		if (stagingImageBuffer.valid()) {
			...
			if (stagingImageBuffer.get_memory().map(0, 0, vk_imageBufferSize, reinterpret_cast<void**>(&pau8StagingBufferContent))) {
				...
				if (create_vulkan_image(
					...
					PRINT_DEBUG("Destroying Vulkan image ", pVulkanTexture->vk_hImage, " and freeing its memory on GPU due to failure allocating essential resources for the texture");
					transferFence.wait_for();
