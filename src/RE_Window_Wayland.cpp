...
namespace RE {
	...
	static void window_base_ping_handler(void *const pData, xdg_wm_base *const wl_pBase, const uint32_t u32Serial) {
		PUSH_TO_CALLSTACKTRACE(xdg_wm_base_pong(wl_pBase, u32Serial));
	}
	...
	void registry_handle_global(void *const pData, wl_registry *const wl_pRegistry, const uint32_t u32Name, const char * const pacInterface, const uint32_t u32Version) {
		if (are_string_contents_equal(pacInterface, wl_compositor_interface.name))
			PUSH_TO_CALLSTACKTRACE(wl_pCompositor = static_cast<wl_compositor*>(wl_registry_bind(wl_pRegistry, u32Name, &wl_compositor_interface, u32Version)));
		else if (are_string_contents_equal(pacInterface, xdg_wm_base_interface.name))
			PUSH_TO_CALLSTACKTRACE(xdg_pWindowBase = static_cast<xdg_wm_base*>(wl_registry_bind(wl_pRegistry, u32Name, &xdg_wm_base_interface, u32Version)));
	}
		...
		if (wl_pDisplay) {
			...
			if (wl_pCompositor) {
				if (xdg_pWindowBase) {
					...
					PUSH_TO_CALLSTACKTRACE(wl_pSurface = wl_compositor_create_surface(wl_pCompositor));
					if (wl_pSurface) {
						PUSH_TO_CALLSTACKTRACE(xdg_pSurface = xdg_wm_base_get_xdg_surface(xdg_pWindowBase, wl_pSurface));
						if (xdg_pSurface) {
							PUSH_TO_CALLSTACKTRACE(xdg_pToplevel = xdg_surface_get_toplevel(xdg_pSurface));
							if (xdg_pToplevel) {
	...
	void wayland_window_proc() {
		//while (PUSH_TO_CALLSTACKTRACE_AND_RETURN(wl_display_dispatch(wl_pDisplay), int32_t) > 0) {
		//}
	}
