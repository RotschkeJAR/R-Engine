...
namespace RE {
	...
	VkPipeline vk_hGraphicsPipeline3D = VK_NULL_HANDLE;

	bool create_graphics_pipeline_3D(const VkShaderModule vk_hVertexShader, const VkShaderModule vk_hFragmentShader) {
		PRINT_DEBUG("Creating Vulkan graphics pipeline for 3D rendering");
		const VkPipeline vk_hPreviousPipeline = vk_hGraphicsPipeline3D;
		...
		if (vkCreateGraphicsPipelines(vk_hDevice, VK_NULL_HANDLE, 1, &vk_createInfo, nullptr, &vk_hGraphicsPipeline3D) == VK_SUCCESS) {
			PRINT_DEBUG("Destroying old Vulkan graphics pipeline ", vk_hPreviousPipeline, " used for 3D rendering");
			vkDestroyPipeline(vk_hDevice, vk_hPreviousPipeline, nullptr);
			return true;
		} else
			RE_ERROR("Failed to create Vulkan graphics pipeline used for 3D rendering");
		return false;
	}

	void destroy_graphics_pipeline_3D() {
		PRINT_DEBUG("Destroying Vulkan graphics pipeline ", vk_hGraphicsPipeline3D, " used for 3D rendering");
